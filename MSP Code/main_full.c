...
volatile float kp = 0.05; //proportional gain was 0.05
volatile float ki = 0.2; //integral gain was 0.2
volatile float error;//error
volatile float accError; //integral error

float dutyCycle = .075;
int PWM2 = 0;
volatile float elbow;
...
void main(void)
{
    ...
    GPIO_setAsInputPinWithPullUpResistor(GPIO_PORT_P1, GPIO_PIN1);  // Button 1
    GPIO_setAsInputPinWithPullUpResistor(GPIO_PORT_P1, GPIO_PIN4);  // Button 2
    ...
    ///// *** Configure Timer A1 and its interrupt *** /////
    Timer_A_configureUpMode(TIMER_A1_BASE, &upConfig_1);
    Interrupt_enableInterrupt(INT_TA1_0);
    Timer_A_startCounter(TIMER_A1_BASE, TIMER_A_UP_MODE);

    ///// *** Configure Timer A2 and its interrupt *** /////
    ...
    TA2CCR1 = 0 ;    // Set duty cycle on pin 5.6 to 7.5%
...
void TA0_0_IRQHandler(void){
    ...
    dutyCycle = ((float) -1*elbow/1982) + 0.131;
    //dutyCycle = ((float) elbow/1982) + 0.0175;
    PWM2 = 60000*dutyCycle;
    TA2CCR1 = PWM2; // Save new PWM value to register
    //printf("PWM Value: %i \r\n\n", PWM2);
...
// Interrupt Service Routine for Port 5
...
void PORT1_IRQHandler(void){
    char Button1 = GPIO_getInputPinValue(GPIO_PORT_P1, GPIO_PIN1);
    char Button2 = GPIO_getInputPinValue(GPIO_PORT_P1, GPIO_PIN4);

    if (Button1 == GPIO_INPUT_PIN_LOW){
        Interrupt_enableInterrupt(INT_TA0_0);
    }
    if (Button2 == GPIO_INPUT_PIN_LOW){
    ...
    GPIO_clearInterruptFlag(GPIO_PORT_P1, GPIO_PIN1);  //Button 1
    GPIO_clearInterruptFlag(GPIO_PORT_P1, GPIO_PIN4);   //Button 2
}
